...
typedef ssize_t (*fr_decode_value_dbuff_t) (TALLOC_CTX *ctx, fr_dcursor_t *cursor, fr_dict_t const *dict,
					    fr_dict_attr_t const *parent,
...
ssize_t fr_struct_from_network_dbuff(TALLOC_CTX *ctx, fr_dcursor_t *cursor,
				     ...
				     void *decoder_ctx,
				     fr_decode_value_dbuff_t decode_value, fr_decode_value_dbuff_t decode_tlv) CC_HINT(nonnull(2,3,4));
...
fr_pair_t *fr_raw_from_network_dbuff(TALLOC_CTX *ctx, fr_dict_attr_t const *parent,
				    fr_dbuff_t *dbuff, size_t data_len) CC_HINT(nonnull);
